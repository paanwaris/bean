...
The ellipsoid-based niche framework that `bean` builds on was developed in the **nicheR** package; without nicheR, this package would not exist. The `bean_ellipsoid` class returned by `fit_ellipsoid()` is designed to be directly usable by nicheR's `predict()` method. If you use `bean` in published work, please cite **nicheR** as well:
