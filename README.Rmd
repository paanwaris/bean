...
```{r fit-ellipse-part2, rgl=TRUE, fig.cap="3D Deterministic Ellipsoid"}
# Fit an ellipse that contains 95% of the thinned data
...
```
```{r calibrate-bean-2, warning=FALSE, message=FALSE, echo=FALSE, results='hide'}
set.seed(81)
...
```
```{r calibrate-bean-4, warning=FALSE, message=FALSE, fig.width=14, fig.height=8}
...
colnames(background_df) <- c("x", "y")
```
```{r calibrate-bean-6, warning=FALSE, message=FALSE, echo=FALSE, results='hide'}
# Calibrate all key parameters
# Note: In a real analysis, use higher thinning_reps and n_repeats.
...
```
```{r calibrate-bean-8, warning=FALSE, message=FALSE, fig.width=14, fig.height=8}
# Print the summary table to see the best combination of parameters
