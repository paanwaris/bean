...
# Ensure rgl runs headlessly in non-interactive sessions
if (!interactive()) {
...
The `plot_bean_nd()` function provides a powerful way to visualize the effect of thinning by overlaying the thinned points on the original data within the environmental grid.
...
```
```{r calibrate-bean-2, warning=FALSE, message=FALSE, fig.width=14, fig.height=8}
